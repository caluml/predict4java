...
public class TLE {
   ...
   */
  public TLE(final String[] tle, final boolean nilStart) {
    ...
    if (tle.length != 3) {
      throw new IllegalArgumentException(String.format("TLE had %d elements", tle.length));
    }
  ...
  private synchronized void preProcessTLESet() {
    double temp;
    temp = TWO_PI / MINS_PER_DAY / MINS_PER_DAY;
    xno = xno * temp * MINS_PER_DAY;
    xndt2o *= temp;
    ...
    deepspace = TWO_PI / xnodp / MINS_PER_DAY >= 0.15625;
  }
  ...
  private String createIllegalArgumentMessage(final int lineCount, final String problem) {
    return String.format("TLE line[%d] %s", lineCount, problem);
  }

  public static List<TLE> importSat(final InputStream inputStream) {
    final List<TLE> importedSats = new ArrayList<>();
  ...
  public boolean equals(Object o) {
    ...
    TLE tle = (TLE) o;
    return catnum == tle.catnum &&
        setnum == tle.setnum &&
        year == tle.year &&
        Double.compare(tle.refepoch, refepoch) == 0 &&
        Double.compare(tle.incl, incl) == 0 &&
        Double.compare(tle.raan, raan) == 0 &&
        Double.compare(tle.eccn, eccn) == 0 &&
        Double.compare(tle.argper, argper) == 0 &&
        Double.compare(tle.meanan, meanan) == 0 &&
        Double.compare(tle.meanmo, meanmo) == 0 &&
        Double.compare(tle.drag, drag) == 0 &&
        Double.compare(tle.nddot6, nddot6) == 0 &&
        Double.compare(tle.bstar, bstar) == 0 &&
        orbitnum == tle.orbitnum &&
        Double.compare(tle.epoch, epoch) == 0 &&
        Double.compare(tle.xndt2o, xndt2o) == 0 &&
        Double.compare(tle.xincl, xincl) == 0 &&
        Double.compare(tle.xnodeo, xnodeo) == 0 &&
        Double.compare(tle.eo, eo) == 0 &&
        Double.compare(tle.omegao, omegao) == 0 &&
        Double.compare(tle.xmo, xmo) == 0 &&
        Double.compare(tle.xno, xno) == 0 &&
        deepspace == tle.deepspace &&
        Objects.equals(name, tle.name) &&
        Objects.equals(createddate, tle.createddate);
  }

  @Override
  public int hashCode() {
    return Objects.hash(catnum, name, setnum, year, refepoch, incl, raan, eccn, argper, meanan, meanmo, drag, nddot6, bstar, orbitnum, epoch, xndt2o, xincl, xnodeo, eo, omegao, xmo, xno, deepspace, createddate);
  }
