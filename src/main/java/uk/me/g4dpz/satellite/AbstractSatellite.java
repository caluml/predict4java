...
abstract class AbstractSatellite implements Satellite {
  ...
  static final double EARTH_RADIUS_KM = 6.378137E3;
  static final double XKE = 7.43669161E-2;
  static final double CK2 = 5.413079E-4;
  /**
   * J3 Harmonic (WGS '72).
   */
  static final double J3_HARMONIC = -2.53881E-6;
  ...
  static final double CK4 = 6.209887E-7;
  protected static final double S = 1.012229;
  private static final double QOMS2T = 1.880279E-09;
  private static final double EARTH_ROTATIONS_PER_SIDERIAL_DAY = 1.00273790934;
  private static final double MFACTOR = 7.292115E-5;
  private static final double ASTRONOMICAL_UNIT = 1.49597870691E8;

  private static final double PERIGEE_156_KM = 156.0;
   ...
   */
  private static final double SOLAR_RADIUS = 6.96000E5;
  ...
  AbstractSatellite(final TLE tle) {
    this.tle = tle;
   ...
   *
   * @param instant the date we want to get the offset for
   * @return the number of days offset
   */
  static double calcDaynum(final Instant instant) {
    final long millis = instant.toEpochMilli() - Instant.parse("1979-12-31T00:00:00Z").toEpochMilli();
    return millis / (double) TimeUnit.DAYS.toMillis(1);
  }
   ...
   *
   * @param location the ground station position
   * @param instant  the date
   */
  @Override
  public synchronized SatPos getPosition(final GroundStationPosition location, final Instant instant) {

    /* This is the stuff we need to do repetitively while tracking. */
    satPos = new SatPos();

    julUTC = AbstractSatellite.calcDaynum(instant) + 2444238.5;
    ...
    calculateObs(julUTC, position, velocity, location, squintVector);
    /* Calculate satellite Lat North, Lon East and Alt. */

    calculateLatLonAlt(julUTC, position, satPos);

    satPos.setTime(instant);
   ...
   */
  @Override
  public boolean willBeSeen(final GroundStationPosition location) {
    ...
    if (tle.getMeanmo() < 1e-8) {
      ...
      return Math.acos(EARTH_RADIUS_KM
          ...
          .abs(location.getLatitude() * DEG2RAD);
    }
   ...
   */
  void setPerigee(final double perigee) {
  ...
  static class Vector4 {
    ...
    final void multiply(final double multiplier) {
      this.x *= multiplier;
      this.y *= multiplier;
      this.z *= multiplier;
    }

    final void setXYZ(final double xValue, final double yValue, final double zValue) {
      this.x = xValue;
      this.y = yValue;
      this.z = zValue;
    }
    ...
    static Vector4 scalarMultiply(final Vector4 vector, final double multiplier) {
      return new Vector4(
     ...
     */
    static double angle(final Vector4 v1, final Vector4 v2) {
     ...
     */
    static Vector4 subtract(final Vector4 v1, final Vector4 v2) {
    ...
    final void setW(final double w) {
      this.w = w;
    }
   ...
   */
  @Override
  public synchronized void calculateSatelliteVectors(final Instant date) {
    ...
    julUTC = AbstractSatellite.calcDaynum(date) + 2444238.5;
   ...
   *
   * @param location The position of the ground station to perform the calculations for.
   * @return satPos The SatPos object where the position of the satellite is stored, as seen from
   * a ground station.
   */
  @Override
  public synchronized SatPos calculateSatPosForGroundStation(final GroundStationPosition location) {
    final Vector4 squintVector = new Vector4();
    // All angles in rads. Distance in km. Velocity in km/s
    // Calculate satellite Azi, Ele, Range and Range-rate
    calculateObs(julUTC, position, velocity, location, squintVector);
  ...
  void calculatePhase(final double xlt, final double xnode, final double omgadf) {
    /* Phase in radians */
  ...
  void calculatePositionAndVelocity(final double rk,
